...
def solve_tsp_heuristic(
    ...
    start_indices = [0] + random.sample(
        range(1, len(points)), min(num_starts - 1, len(points) - 1)
    )
...
def get_distance(
    distances: Dict[Tuple[str, str], int], from_point: str, to_point: str
) -> Optional[int]:
...
def calculate_path_distance(
    distances: Dict[Tuple[str, str], int], path: List[str]
) -> Optional[int]:
...
def improve_with_2opt(
    distances: Dict[Tuple[str, str], int], path: List[str], max_iterations: int = 1000
) -> List[str]:
    """
...
def improve_with_3opt(
    distances: Dict[Tuple[str, str], int], path: List[str], max_iterations: int = 100
) -> List[str]:
    """
        ...
        for i in range(n - 2):
            for j in range(i + 2, n - 1):
                for k in range(j + 2, n):
                    ...
                    d_current = (
                        get_distance(distances, best_path[i], best_path[i + 1])
                        or 0 + get_distance(distances, best_path[j], best_path[j + 1])
                        or 0
                        + get_distance(distances, best_path[k], best_path[(k + 1) % n])
                        or 0
                    )
                    ...
                    d_new = (
                        get_distance(distances, new_path[i], new_path[i + 1])
                        or float("inf")
                        + get_distance(distances, new_path[j], new_path[j + 1])
                        or float("inf")
                        + get_distance(distances, new_path[k], new_path[(k + 1) % n])
                        or float("inf")
                    )
